...
While 1 ; lặp tìm phòng game nếu ở sảnh nè
	...
	While 1 ; bắt đầu lặp tìm game
		...
		While 1
			...
			If $error1*$error2 <> 1 Then
				...
				Sleep(500) ; nghỉ 500s để tool cập nhật nè.
	...
	;~ 				While $bolech < 35
	;~ 					ControlSend($hWndgunny,"","",$quaynguoc_huongdich) ; thao tác đầu game tăng góc nè , bò ngược lại nè, vượt chướng ngại vật nè
	;~ 					$bolech+=1
	;~ 				WEnd

					While $tanggoc < 25 ; tăng max góc đầu game cho dễ chơi nè
				...
				ControlSend($hWndgunny,"","",$quay_huongdich)

				_Check_goc($hWndgunny,$hWndtool,$quay_huongdich,$quaynguoc_huongdich)

				ControlSend($hWndgunny,"","","1478b")

				Sleep(500) ; cho nó nghỉ xíu xìu xiu chứ nó chạy nhanh quá bên tool ko cập nhật kịp được nè
